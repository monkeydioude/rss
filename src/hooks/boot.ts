...
import { setChannels, useDispatch as useChannelsDispatch } from "src/states/channels";
import { setFeed, useDispatch as useFeedDispatch } from "src/states/feed";
...
const useBoot = (onBootFinish?: () => void): boolean => {
    ...
    const loadLocalChannels = async (): Promise<Map<number, string>> => {
        try {
            ...
            const res = await (new JSONStorage<Map<number, string>>(appConfig.storageKeys.channel_ids)).retrieve();
